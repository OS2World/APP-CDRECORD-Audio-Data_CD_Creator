...
static BOOL checkMkisofsExists(HWND hwnd, HMODULE hResource)
{
...
void CWDataFolder::cwWriteOnTheFly()
{
  ...
  if(!(ulFlags & IDCDT_FIXDISK) && (ulFlags & IDCDT_USERDEFINED)) {
    /* Text: "Fix disk is unchecked!\n " */
    /* Title: "Write image." */
    if(showMsgBox2(hwndMkisofsMain, IDSTRD_WRITEIMAGETITLE, IDSTRD_FIXUNCHECKED, hDataResource, MB_OKCANCEL | MB_ICONEXCLAMATION|MB_MOVEABLE)
       ==MBID_CANCEL)
      return;
  }
  ...
  if(sDummy!=1)
    {
      ...
      if(showMsgBox2(hwndMkisofsMain, IDSTRD_ONTHEFLYTITLE, IDSTR_DUMMYUNCHECKED, hDataResource, MB_YESNO | MB_ICONEXCLAMATION|MB_MOVEABLE)
         ==MBID_NO)
        return;
    }
