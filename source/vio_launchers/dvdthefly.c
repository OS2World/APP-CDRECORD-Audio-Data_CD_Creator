...
void message()
{
  printf("This program is (C) Chris Wohlgemuth 1999-2004\n");
...
int main(int argc, char * argv[])
{
  ...
  *mem=0;
  ...
  /* FIFO size. Needing it to know when FIFO is filled. We raise the priority of the mkisofs
     process then.
     */
  if((fifoPtr=strstr(pipePtr,"-b "))!=NULLHANDLE) {
    fifoPtr+=2;
    iFifo=atoi(fifoPtr);
    iFifo*=1024*248; /* Fifo in Bytes */
  }        
        ...
        if(!DosCreatePipe(&hpR2, &hpW2, PIPESIZE)) {
          if(!DosDupHandle(hpW2, &hfNew2)){/* Duplicates standard out handle */
            ...
            do {
              ...
              delta+=cbRead2;
              writtenB+=cbRead2;
              if(delta>=PIPESIZE*800) {
                delta=0;
                WinPostMsg(hwndNotify,WM_APPTERMINATENOTIFY,MPFROMLONG(ACKEY_MBWRITTEN),MPFROMLONG((ULONG)(writtenB/2048)));
              }
              /* Increase process priority adter FIFO is filled */
